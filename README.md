...
![image-20240827023518010](./assets/image-20240827023518010.png)
...
2. 阅读其中 `g()`, `f()` 和 `main()` 函数的代码
   ...
   ![image-20240827223955034](./assets/image-20240827223955034.png)
...
![image-20240827224800959](./assets/image-20240827224800959.png)
...
4. 添加 `backtrace()` 函数原型到 `kernel/defs.h`
   ...
   ![image-20240827231339582](./assets/image-20240827231339582.png)

5. 在 `kernel/printf.c` 的 `panic()` 函数中添加对 `backtrace()` 的调用.

   ![image-20240827231559699](./assets/image-20240827231559699.png)
...
6. 在kernel/sysproc.c编写 sys_sigalarm() 函数, 将 interval 和 handler 的值存到当前进程的 struct proc 结构体的相应字段中.
   ...
   ![image-20240827235404654](./assets/image-20240827235404654.png)
...
7. `kernel/proc.c` 的 `allocproc()` 函数负责分配并初始化进程, 
   ...
   ![image-20240827235653492](./assets/image-20240827235653492.png)
...
8. 每经过异常时钟间隔, 会引发时钟中断,调用 kernel/trap.c 中的 usertrap() 函数. 对于时钟中断 which_dev 变量的值为 2, 由此便可以单独对时钟中断进行操作.
   ...
   ![image-20240828000741041](./assets/image-20240828000741041.png)
...
15. 实验结果
    ...
    ![image-20240828005049914](./assets/image-20240828005049914.png)
...
![image-20240828011203851](./assets/image-20240828011203851.png)
